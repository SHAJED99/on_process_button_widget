...
| `onDone` | `void Function(bool?)?` | `null` | Called after `onTap` and status display completion. |
...
| `fontColor` | `Color?` | `Theme onPrimary` | Color of the button text. |
| `iconColor` | `Color?` | `Theme onPrimary` | Color of status icons and child icons. |
| `fontWeight` | `FontWeight?` | `bold` | Font weight of button text. |
| `textStyle` | `TextStyle?` | `titleMedium` | Full text style override. |
| `border` | `BoxBorder?` | `null` | Border around the button (set for outlined style). |
| `contentPadding` | `EdgeInsetsGeometry?` | `horiz:12, vert:4` | Inner padding around content. |
| `showRunningStatusWidget` | `bool?` | `true` | Whether to show the running status widget. |
| `roundBorderWhenRunning` | `bool?` | `true` | Whether to use circular border during status states. |
| `expandedIcon` | `bool?` | `null` | Whether the status icon expands to fill width. |
| `statusShowingDuration` | `Duration?` | `2 seconds` | How long success/error status is displayed. |
| `onStatusChange` | `void Function(BuildContext?, OnProcessButtonStatus)?` | `null` | Called when button status changes. |
